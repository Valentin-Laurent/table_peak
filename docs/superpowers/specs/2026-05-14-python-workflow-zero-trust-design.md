> **⚠️ SUPERSEDED 2026-05-25** by [`2026-05-25-uv-to-pdm-migration-design.md`](2026-05-25-uv-to-pdm-migration-design.md).
>
> The Path-A decision below (disable the macOS sandbox to keep uv) is reversed. The replacement spec keeps the sandbox enabled and replaces uv with pdm. The Makefile wrapper and broader-matcher decisions in this spec remain in force — only the sandbox-layer and tool-choice decisions change.
>
> This file is preserved for the diagnostic record: the `SCDynamicStoreCreate` panic location, the layered-problem framing, and the rationale for why per-call sandbox bypass is not viable under `dontAsk` remain useful context.
